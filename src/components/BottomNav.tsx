...
const items = [
  ...
  { href: "/claims/new", label: "Damage", icon: "📸" },
        ...
        {items.map((it) => {
          ...
          return (
            <Link
              key={it.href}
              href={it.href}
              className={`flex flex-col items-center gap-0.5 py-2.5 text-[11px] font-medium transition-all duration-300 ${
                active ? "text-clay-700 scale-105" : "text-stone-400 hover:text-clay-500"
              }`}
            >
              <span className={`text-lg leading-none ${active ? "drop-shadow-[0_2px_6px_rgba(198,77,48,0.4)]" : ""}`}>
                {it.icon}
              </span>
              {it.label}
              <span
