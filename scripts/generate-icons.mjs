...
async function cleanLogo() {
  ...
  // 2. Erode 2 rings of near-white bordering the exterior — eats the halo.
  for (let pass = 0; pass < 2; pass++) {
    const grow = [];
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        const p = y * width + x;
        if (outside[p] || !isWhite(idx(x, y))) continue;
        if (
          (x > 0 && outside[p - 1]) ||
...
/** SVG for a soft light background (icons look clean with the red/grey logo). */
const iconBg = (size, round) => `
<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}">
  <defs>
    <linearGradient id="g" x1="0" y1="0" x2="1" y2="1">
      <stop offset="0" stop-color="#ffffff"/>
      <stop offset="1" stop-color="#fdf1ee"/>
    </linearGradient>
  </defs>
...
async function makeScreenshot(logoPng, w, h, file) {
  const bg = Buffer.from(`
    <svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}">
      ...
      <rect x="0" y="0" width="${w}" height="${Math.round(h * 0.020)}" fill="#8a3622"/>
      ...
      <text x="${w / 2}" y="${h * 0.70}" text-anchor="middle" font-family="Segoe UI, Arial, sans-serif"
            font-size="${Math.round(Math.min(w, h) * 0.030)}" fill="#a8746a">Production · Assets · Sales · Finance</text>
    </svg>`);
  const logo = await sharp(logoPng)
